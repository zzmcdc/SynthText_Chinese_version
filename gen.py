...
def add_res_to_db(imgname, res, db):
  ...
  for i in xrange(ninstance):
    ...
    db['data'][dname].attrs.create(
        'txt', res[i]['txt'], dtype=h5py.special_dtype(vlen=unicode))
...
def main(num_worker, viz=False):
  ...
  def worker(img, imname, depth, seg, area, label):
    res = RV3.render_text(img, depth, seg, area, label, ninstance=INSTANCE_PER_IMAGE, viz=False)
    for ct in range(5):
      if len(res) > 0:
  ...
  for i in xrange(start_idx, end_idx):
    ...
    pool.apply_async(worker, args=(img, imname, depth, seg, area, label))
    time.sleep(0.1)
  pool.close()
  pool.join()
...
if __name__ == '__main__':
  ...
  parser.add_argument('--num_work', type=int, default=8, help='number worker ')
  args = parser.parse_args()
  main(args.num_work, args.viz)
